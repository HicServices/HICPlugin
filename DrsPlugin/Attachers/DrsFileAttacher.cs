...
namespace DrsPlugin.Attachers
{
    ...
    {        
        ...
        private void UpdateRAWWithImageArchiveLocations(IDataLoadJob job, Dictionary<string, List<string>> archiveMappings)
        {
            job.OnNotify(this,
                new NotifyEventArgs(ProgressEventType.Information,
                    "Updating [" + ImageArchiveUriColumnName +
                    "] in RAW with the correct location of each image in the DRS Image Archive"));
            ...
            foreach (var kvp in archiveMappings)
            {
                ...
                var query = "UPDATE [" + TableName + "] SET " + ImageArchiveUriColumnName + " = CONCAT('" + kvp.Key + "!', " +
                            FilenameColumnName + ") " +
        ...
        public override void LoadCompletedSoDispose(ExitCodeType exitCode, IDataLoadEventListener listener)
        {
            ...
            if (SecureLocalScratchArea.EnumerateFileSystemInfos().Any())
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "SecureLocalScratchArea is not empty - a successful load should result in an empty directory at " + SecureLocalScratchArea.FullName));
        }
