...
namespace DrsPlugin.Attachers
{
    public class ImageIntegrityChecker
    {
        ...
        {
            listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Comparing images in " + pathToStrippedFiles + " to " + pathToOriginalFiles));
            ...
            foreach (var strippedImage in Directory.EnumerateFiles(pathToStrippedFiles))
            {
                ...
                if (filename == null)
                    throw new InvalidOperationException("Could not retrieve filename from stripped image path: " + strippedImage);

                var file = Directory.EnumerateFiles(pathToOriginalFiles, filename, SearchOption.AllDirectories).SingleOrDefault();
                if (file == null)
                    throw new FileNotFoundException("Could not find original file " + strippedImage + " in " + pathToOriginalFiles);
        ...
        {
            listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Stripped images in " + pathToStrippedFiles));
        ...
        private static void CompareStreams(IImagePatcher patcher, Stream originalImageStream, Stream strippedImageStream)
        {
            ...
            if (originalPixels.Length == strippedPixels.Length)
                ...
                additional = "The pixel byte array lengths are different. Original = " + originalPixels.Length + ", Stripped = " +
                             strippedPixels.Length;

            throw new InvalidOperationException("The EXIF stripping process appears to have altered the pixel data. " +
                                                additional);
        }
