...
namespace DrsPlugin.Attachers
{
    public class DrsMultiVolumeRarAttacher : Attacher, IPluginAttacher
    {
        ...
        private ExtractedMultiVolumeRarProvider CreateExtractedArchiveProvider(DirectoryInfo workingDir, IDataLoadEventListener listener)
        {
            ...
            {
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Using existing image files in " + _existingExtractionDirectory));
        ...
        private void Process(IArchiveProvider archiveProvider, IDataLoadJob job)
        {
            ...
            job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Creating temporary directory at " + tempDirPath));
            ...
            job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Archiving images (created archives will be moved to " + ArchivePath.FullName + " once the load is successfully completed."));
        ...
        private void ArchiveImagesForStorage(IDataLoadJob job, DirectoryInfo tempDir)
        {
            ...
            job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Updating [" + ImageArchiveUriColumnName + "] in RAW with the correct location of each image in the DRS Image Archive"));
            ...
            foreach (var kvp in archiveMappings)
            {
                ...
                var query = "UPDATE [" + TableName + "] SET " + ImageArchiveUriColumnName + " = CONCAT('" + kvp.Key + "!', " + FilenameColumnName + ") " +
                            "WHERE " + FilenameColumnName + " IN ('" + string.Join("','", kvp.Value) + "')";
        ...
        public override void Check(ICheckNotifier notifier)
        {
            ...
            {
                notifier.OnCheckPerformed(new CheckEventArgs("No files found in ForLoading: " + LoadDirectory.ForLoading.FullName, CheckResult.Fail));
                return;
            }
            ...
            {
                notifier.OnCheckPerformed(new CheckEventArgs("No manifest file found in ForLoading: " + LoadDirectory.ForLoading.FullName, CheckResult.Fail));
                return;
            }
        ...
        private void CheckImageArchive(IArchiveProvider archiveProvider, ICheckNotifier notifier)
        {
            ...
            if (unexpectedFileExtensions.Any())
                notifier.OnCheckPerformed(new CheckEventArgs("Unexpected file extensions in " + archiveProvider.Name + ": " + string.Join(",", unexpectedFileExtensions), CheckResult.Fail));
        ...
        private void CheckManifestAgreement(IArchiveProvider archiveProvider, ICheckNotifier notifier)
        {
            ...
            if (notInArchive.Any())
                notifier.OnCheckPerformed(new CheckEventArgs("These files are specified in the manifest but are not present in the archive: " + string.Join(",", notInArchive), CheckResult.Fail));

            var notInManifest = imageFilenamesInArchive.Except(imageFilenamesInManifest).ToList();
            if (notInManifest.Any())
                notifier.OnCheckPerformed(new CheckEventArgs("These files are present in the archive but are not specified in the manifest: " + string.Join(",", notInManifest), CheckResult.Fail));
        }
        ...
        public override void LoadCompletedSoDispose(ExitCodeType exitCode, IDataLoadEventListener listener)
        {
            ...
            listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "The remainder of the load has completed successfully, now moving all archived images to the DRS Image Archive at " + ArchivePath.FullName));
