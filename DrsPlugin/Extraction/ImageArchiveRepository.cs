...
public class ImageArchiveRepository
{
    ...
    public void CopyToStream(string archiveUri, Stream outputStream)
    {
        ...
        if (!File.Exists(archiveLocation))
            throw new InvalidOperationException(
                $"The archive {uriParts[0]} does not exist at the expected location of {archiveLocation}");
        ...
        if (archiveType == ".zip")
            ...
            throw new InvalidOperationException(
                $"Unsupported type of image archive: '{archiveType}'. Image archives must either be .zip or .tar files.");
    }
    ...
    private void CopyToStreamFromZip(string archiveLocation, string fileName, Stream outputStream)
    {
        using (var archive = ZipFile.Open(archiveLocation, ZipArchiveMode.Read))
        {
            ...
            if (entry == null)
                throw new InvalidOperationException(
                    $"The archive {archiveLocation} does not contain the file {fileName}");
    ...
    private void ValidateUriString(string uri)
    {
        if (!uri.Contains("!"))
            throw new InvalidOperationException(
                $"{uri} is not a valid RDMP Image Archive file URI (should be archive_filename!image_filename)");
