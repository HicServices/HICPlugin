...
namespace DrsPlugin.Extraction
{
    public abstract class ImageExtraction : IPluginDataFlowComponent<DataTable>, IPipelineRequirement<IExtractCommand>
    {
        ...
        protected bool PreProcessingCheck(IDataLoadEventListener listener)
        {
            ...
            {
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, "Ignoring batch because it does not come from a image bundle (DatasetName). Table name was: " + datasetName + ", didn't match regex: " + DatasetName));
                return false;
            }
