        ...
        public override ExitCodeType Attach(IDataLoadJob job,GracefulCancellationToken token)
        {
            ...
            destRepo.InsertionError += (sender, e, query) => job.OnNotify(sender,
                new NotifyEventArgs(ProgressEventType.Warning,
                    "Received InsertionError from " + typeof(SciStoreDbRepository).Name + " " + e +
                    Environment.NewLine + "Query: " + query, e));
            ...
            try
            {
                ...
                job.OnNotify(this,
                    new NotifyEventArgs(ProgressEventType.Information, "Reading the CombinedReport XML files from " + LoadDirectory.ForLoading));
                ...
                catch (BadCombinedReportDataException e)
                {
                    ...
                    job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Error, "Bad report detected when running attacher: " + e, e));
                    var archiveFilepath = srcRepo.FindArchiveContainingReport(e.BadReport);
                    job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Error, "Bad report lives in archive at: " + archiveFilepath));
                    throw;
                }
                ...
                job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, destRepo.HeadersTable.Rows.Count + " rows added to Header Data Table"));
                job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, destRepo.SampleDetailsTable.Rows.Count + " rows added to Sample Details Data Table"));
                job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Information, destRepo.ResultsTable.Rows.Count + " rows added to Results Data Table"));
            ...
            {
                job.OnNotify(this, new NotifyEventArgs(ProgressEventType.Error, typeof (SciStoreAttacher).Name + ".Attach() failed ", e));
        ...
        private void ResolveTestResultOrderDuplication(List<SciStoreReport> labs, IDataLoadEventListener job)
        {
            foreach (var lab in labs)
            {
                foreach (var sample in lab.Samples)
                {
                    ...
                    if (recordsRemoved > 0)
                        job.OnNotify(this,new NotifyEventArgs(ProgressEventType.Warning,"Resolved duplicate TestResultOrder using method 'ResolveTestResultOrderDuplication' in lab number:" +lab.Header.LabNumber ));
                }
            }
        }
    }
