...
namespace SCIStorePlugin.Data
{
    public class TestSetFactory
    {
            ...
            foreach (var clinicalCircumstanceType in testSetDetails.TestName)
            {
                ...
                if (asString != null)
                {
                    if (testSet.ClinicalCircumstanceDescription != null)
                        throw new Exception("Multiple <ClinicalCircumstanceDescription>. Encountered: " + asString + ", but have already seen: " + testSet.ClinicalCircumstanceDescription);
        ...
        private void PopulateAppropriateCodeField(TestSet testSet, ClinicalCodeInfo clinicalInformationList,IDataLoadEventListener listener)
        {
            switch (clinicalInformationList.SchemeId)
            {
                ...
                case ClinicalCodeScheme.Undefined:
                    ...
                    {
                        listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Encountered something that was not a readcode ('" + clinicalInformationList.Value + "') but we had already encountered a nonreadcode(" + testSet.LocalCode.Value + ") - so we are just going to treat it like a readcode anyway"));
                        testSet.ReadCode = clinicalInformationList;
                    }
                    break;
                default:
        ...
        private void DetermineWhichLooksMostLegitAndAssign(TestSet testSet, ClinicalCodeInfo readCode1, ClinicalCodeInfo readCode2,IDataLoadEventListener listener)
        {
            ...
            listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Two valid read codes found in TestSet: " + readCode1.Value + ", " + readCode2.Value + "."));
            ...
            if (_highlySuspectSupposedReadCodes.Contains(kvp1))
            {
                ...
                if (_highlySuspectSupposedReadCodes.Contains(kvp2))
                    throw new Exception("We have found two potential read codes for this TestSet, however they both look dodgy: " + readCode1.Value + ", " + readCode2.Value);

                // readCode2 looks ok, go with that
                testSet.ReadCode = readCode2;
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Have chosen " + readCode2.Value + " (" + readCode2.Description + ")"));
                ...
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Local code is null, so using what looks like a dodgy read code but is probably a local code: " + readCode1.Value + " (" + readCode1.Description + ")"));
            ...
            else
            {
                ...
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Have chosen " + readCode1.Value + " (" + readCode1.Description + ")"));
                if (testSet.LocalCode != null) return;

                // if we haven't already set a local code, use readCode2 in case it actually is a local code rather than a read code
                listener.OnNotify(this, new NotifyEventArgs(ProgressEventType.Warning, "Local code is null, so using what looks like a dodgy read code but is probably a local code: " + readCode2.Value + " (" + readCode2.Description + ")"));
